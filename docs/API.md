...
This document provides detailed information about the Azure APIM Authentication
Delegation Function App endpoints.

## 📋 Overview

The function app exposes three HTTP endpoints that handle the APIM delegation
authentication flow:
...
**Methods**: `GET`  
**Purpose**: Processes OAuth callbacks from Okta and completes APIM
authentication
...
async function testDelegationEndpoint() {
  try {
    const response = await axios.get(
      'https://yourapp.azurewebsites.net/api/delegation',
      {
        params: {
          operation: 'SignIn',
          returnUrl: 'test',
